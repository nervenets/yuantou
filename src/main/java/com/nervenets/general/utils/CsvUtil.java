...
public class CsvUtil {
    ...
    public static <T> ResponseEntity<Resource> sendDataStream(List<T> dataList, String fileName, Class<T> dataClass) {
        return sendDataStream(dataList, fileName, null, dataClass, null);
    }

    public static <T> ResponseEntity<Resource> sendDataStream(List<T> dataList, String fileName, Class<T> dataClass, Class<?>... childClasses) {
        return sendDataStream(dataList, fileName, null, dataClass, childClasses);
    }

    public static <T> ResponseEntity<Resource> sendDataStream(List<T> dataList, String fileName, String group, Class<T> dataClass, Class<?>... childClass) {
    ...
    private static Field[] initFields(Class<?> dataClass, Class<?>... childClasses) {
        ...
        for (Class<?> childClass : Arrays.asList(childClasses)) {
    ...
    private static <T> String getCsvOneLine(Map<String, CsvColumn> filedAnnotationMap, T lineData, Class<?> dataClass, Class<?>... childClasses) {
        ...
        filedAnnotationMap.forEach((key, value) -> {
            ...
            try {
                ...
                if (obj != null && Arrays.asList(childClasses).indexOf(obj.getClass()) > -1) {
                    for (Class<?> childClass : Arrays.asList(childClasses)) {
                        if (obj.getClass().equals(childClass)) {
                            Arrays.asList(value.childProperties()).forEach(childProperty -> {
                                try {
                    ...
                    if (StringUtils.isNotEmpty(value.springEL())) {
                        dataColumn = getSpringELValue(value.springEL(), lineData);
                    }
...
//        csvHeadLineCache.put(Integer.toHexString(filedAnnotationMap.hashCode()), csvHeaderLineStr);
        ...
        for (CsvColumn csvColumn : filedAnnotationMap.values()) {
            if (csvColumn.titles() != null && csvColumn.titles().length != 0) {
    ...
    private static String getRealCsvFileName(String fileName) {
        ...
        try {
            fileName = CsvUtil.FILE_PREFIX
                    .concat(fileName.replaceAll(" ", "-").concat("_"))
                    .concat(JodaUtils.timeLongToString());
            fileName = URLEncoder.encode(fileName, "UTF-8");
        } catch (Exception e) {
            log.error("CSV文件名转换错误");
        }
        return fileName;
    }
